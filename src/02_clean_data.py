...
if len(sys.argv) > 1:
    ...
    files = [f for f in os.listdir(INTERMEDIATE_DIR) if f.endswith('.csv') and not f.endswith('_cleaned.csv')]
    if not files:
...
output_file = os.path.join(INTERMEDIATE_DIR, f'{date_str}_cleaned.csv')
...
new_order = [
    "eva_nr", "station_name", "category_train",
    "train", "train_number", "ID",
...
print(f"Bereinigte Tagesdatei erstellt: {output_file}")
